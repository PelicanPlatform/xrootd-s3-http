...
To load the authorization wrapper, invoke `ofs.authlib` in "stacking" mode (`++`).
The deadlock wrapper is added on top of the already-configured authorization
object (the built-in `acc.authdb`-based authorization, or any other authorization
plugin loaded earlier), which it wraps:

```
ofs.authorize 1
acc.authdb /etc/xrootd/authdb
ofs.authlib ++ libXrdAccDeadlock.so
```

The wrapper must not be the first (base) authorization plugin: it needs an
existing authorization object to wrap. Any other authorization plugins should be
loaded before it on their own `ofs.authlib` lines.
